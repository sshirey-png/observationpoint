...
/**
 * WriteUp — formal disciplinary write-up.
 * V3 family, red HR accents, draft paradigm, typed acknowledgment via email.
 */

const FORM_TYPE = 'write_up'
const WARNING_TYPES = ['Verbal Warning', 'Written Warning', 'Final Written Warning']
const CATEGORIES = ['Professionalism', 'Performance', 'Attendance', 'Policy Violation', 'Commitment']
...
export default function WriteUp() {
  ...
  useEffect(() => {
    ...
    async function loadDraft() {
      try {
        ...
        if (fb.incident_date) setIncidentDate(fb.incident_date)
        if (fb.warning_type) setWarningType(fb.warning_type)
        if (Array.isArray(fb.categories)) setCategories(fb.categories)
        if (fb.description) setDescription(fb.description)
        if (fb.prior_discussions) setPriorDiscussions(fb.prior_discussions)
        if (fb.expectations) setExpectations(fb.expectations)
  ...
  }, [teacher, incidentDate, warningType, categories, description, priorDiscussions, expectations, consequences])
  ...
  const requiredFilled = (
    incidentDate && warningType &&
    categories.length > 0 &&
    description.trim() && priorDiscussions.trim() &&
    expectations.trim() && consequences.trim()
  )
      ...
      if (asDraft) {
        ...
        if (mode === 'publish_and_send' && finalId) {
          try {
            ...
            alert('Write-Up published, but email to employee failed: ' + e.message)
          }
        }
        setDone(true)
      }
  ...
  async function abandonDraft() {
    ...
    try {
      ...
      setIncidentDate(''); setWarningType(''); setCategories([])
      setDescription(''); setPriorDiscussions(''); setExpectations(''); setConsequences('')
      setSaveStatus('idle')
    } catch (e) {
      ...
      <FormShell>
        <div style={{ minHeight: '100svh', background: '#f5f7fa', padding: 16, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Inter, sans-serif' }}>
          <div style={{ background: '#fff', borderRadius: 18, padding: 28, textAlign: 'center', maxWidth: 360, boxShadow: '0 8px 24px rgba(0,0,0,.1)' }}>
            ...
            <div style={{ fontSize: 13, color: '#6b7280', marginTop: 8, lineHeight: 1.5 }}>Write-Ups are a formal HR document and can only be filed by Leadership, Network staff, or HR admins.</div>
  ...
  if (done) {
    return (
      <div style={{ minHeight: '100svh', background: '#f5f7fa', padding: 16, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Inter, sans-serif' }}>
        <div style={{ background: '#fff', borderRadius: 18, padding: 28, textAlign: 'center', maxWidth: 360, boxShadow: '0 8px 24px rgba(0,0,0,.1)' }}>
          <div style={{ width: 56, height: 56, borderRadius: '50%', background: '#fee2e2', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 14px', color: '#dc2626', fontSize: 28, fontWeight: 800 }}>✓</div>
          <div style={{ fontSize: 20, fontWeight: 800, color: '#002f60' }}>Write-Up submitted</div>
    ...
    <div style={{ minHeight: '100svh', background: '#f5f7fa', paddingBottom: 'calc(100px + env(safe-area-inset-bottom))', fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ padding: 14, maxWidth: 720, margin: '0 auto' }}>
        ...
        <div style={{ background: '#fef2f2', borderLeft: '4px solid #dc2626', borderRadius: '0 10px 10px 0', padding: '12px 14px', marginBottom: 14, fontSize: 13, color: '#991b1b', lineHeight: 1.5 }}>
          <b>Formal disciplinary document.</b> This Write-Up will be stored in the employee's record.
        </div>

        <div style={sectionTitle}>Employee Write-Up</div>
        <div style={sectionSub}>Document the incident, prior discussions, and category.</div>
        ...
        <div style={{ background: '#fff', borderRadius: 14, padding: 16, boxShadow: '0 1px 3px rgba(0,0,0,.05)', marginBottom: 12 }}>
          ...
          <div style={cardLabel}>Category <span style={{ color: '#dc2626' }}>*</span></div>
            ...
            {CATEGORIES.map(c => {
              ...
              return (
                <label key={c} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 12px', background: on ? '#fee2e2' : '#f5f7fa', borderRadius: 8, fontSize: 13, cursor: 'pointer', color: on ? '#991b1b' : '#374151', fontWeight: on ? 700 : 400 }}>
                  <input type="checkbox" checked={on} onChange={() => toggleCategory(c)} style={{ width: 16, height: 16, accentColor: '#dc2626' }} />
                  {c}
                </label>
              )
            })}
          </div>
          <div style={cardLabel}>Description of Issue <span style={{ color: '#dc2626' }}>*</span></div>
          <textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="Describe the incident or behavior. Include dates, specifics, and context." style={textarea} />
          <div style={cardLabel}>Prior Discussions <span style={{ color: '#dc2626' }}>*</span></div>
          <textarea value={priorDiscussions} onChange={e => setPriorDiscussions(e.target.value)} placeholder="Any prior conversations about this issue. Include dates." style={{ ...textarea, minHeight: 60 }} />
        </div>

        <div style={sectionTitle}>Expectations & Consequences</div>

        <div style={{ background: '#fff', borderRadius: 14, padding: 16, boxShadow: '0 1px 3px rgba(0,0,0,.05)', marginBottom: 12 }}>
          <div style={cardLabel}>Expectations Going Forward <span style={{ color: '#dc2626' }}>*</span></div>
          <textarea value={expectations} onChange={e => setExpectations(e.target.value)} placeholder="What is expected of the employee going forward? Be specific and measurable." style={textarea} />
          <div style={cardLabel}>Consequences if Not Corrected <span style={{ color: '#dc2626' }}>*</span></div>
          <textarea value={consequences} onChange={e => setConsequences(e.target.value)} placeholder="Next steps if behavior continues or expectations are not met." style={{ ...textarea, minHeight: 60 }} />
        </div>
        ...
        <div style={sectionSub}>Attach evidence — emails, prior write-ups, witness statements, photos. Stored in HR-locked storage.</div>
        <div style={{ background: '#fff', borderRadius: 14, padding: 14, marginBottom: 12 }}>
          {draftId ? (
            <FileDrop parentType="touchpoint" parentId={draftId} formType="write_up" />
          ) : (
      ...
      <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: '#fff', borderTop: '1px solid #e5e7eb', padding: '10px 14px', paddingBottom: 'max(14px, env(safe-area-inset-bottom))', zIndex: 50 }}>
        <div style={{ maxWidth: 720, margin: '0 auto', display: 'flex', gap: 6 }}>
          ...
          <button onClick={() => submit('publish')} disabled={!canSubmit}
            title={!teacher ? 'Pick a teacher first' : !canSubmit ? 'Fill required fields' : 'Submit Write-Up internally'}
